...
HAL_StatusTypeDef HAL_I2C_Init(I2C_HandleTypeDef *hi2c)
{
  ...
  SetRegister(&(hi2c->Instance->CR1), GetRegister(&(hi2c->Instance->CR1)) | I2C_CR1_SWRST);  // REWRITE:
  // hi2c->Instance->CR1 &= ~I2C_CR1_SWRST;
  SetRegister(&(hi2c->Instance->CR1), GetRegister(&(hi2c->Instance->CR1)) & ~I2C_CR1_SWRST);  // REWRITE:
...
static void I2C_Flush_DR(I2C_HandleTypeDef *hi2c)
{
  ...
  if (__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_TXE) != RESET)
  {
    ...
    SetRegister(&(hi2c->Instance->DR), 0x00U);  // REWRITE:
  }
}
...
HAL_StatusTypeDef HAL_I2C_Master_Transmit(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    while (hi2c->XferSize > 0U)
    {
      ...
      SetRegister(&(hi2c->Instance->DR), *hi2c->pBuffPtr);  // REWRITE:
    ...
    while (hi2c->XferSize > 0U)
    {
      if (hi2c->XferSize <= 3U)
      {
        ...
        if (hi2c->XferSize == 1U)
        {
          ...
          *hi2c->pBuffPtr = (uint8_t)GetRegister(&(hi2c->Instance->DR));  // REWRITE:
...
HAL_StatusTypeDef HAL_I2C_Slave_Transmit_DMA(I2C_HandleTypeDef *hi2c, uint8_t *pData, uint16_t Size)
{
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    if (dmaxferstatus == HAL_OK)
    {
      ...
      SetRegister(&(hi2c->Instance->CR2), GetRegister(&(hi2c->Instance->CR2)) | I2C_CR2_DMAEN);  // REWRITE:
...
HAL_StatusTypeDef HAL_I2C_IsDeviceReady(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint32_t Trials, uint32_t Timeout)
{
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    do
    {
      ...
      SetRegister(&(hi2c->Instance->DR), I2C_7BIT_ADD_WRITE(DevAddress));  // REWRITE:
...
static void I2C_MemoryTransmit_TXE_BTF(I2C_HandleTypeDef *hi2c)
{
  ...
  if (hi2c->EventCount == 0U)
  {
    ...
    if (hi2c->MemaddSize == I2C_MEMADD_SIZE_8BIT)
    {
      ...
      SetRegister(&(hi2c->Instance->DR), I2C_MEM_ADD_LSB(hi2c->Memaddress));  // REWRITE:
    ...
    else
    {
      ...
      SetRegister(&(hi2c->Instance->DR), I2C_MEM_ADD_MSB(hi2c->Memaddress));  // REWRITE:
  ...
  else if (hi2c->EventCount == 2U)
  {
    if (CurrentState == HAL_I2C_STATE_BUSY_RX)
    {
      ...
      SetRegister(&(hi2c->Instance->CR1), GetRegister(&(hi2c->Instance->CR1)) | I2C_CR1_START);  // REWRITE:
...
static void I2C_Master_SB(I2C_HandleTypeDef *hi2c)
{
  if (hi2c->Mode == HAL_I2C_MODE_MEM)
  {
    if (hi2c->EventCount == 0U)
    {
      ...
      SetRegister(&(hi2c->Instance->DR), I2C_7BIT_ADD_WRITE(hi2c->Devaddress));  // REWRITE:
    }
    else
    {
      // hi2c->Instance->DR = I2C_7BIT_ADD_READ(hi2c->Devaddress);
      SetRegister(&(hi2c->Instance->DR), I2C_7BIT_ADD_READ(hi2c->Devaddress));  // REWRITE:
    }
  }
  else
  {
    ...
    else
    {
      if (hi2c->EventCount == 0U)
      {
        ...
        SetRegister(&(hi2c->Instance->DR), I2C_10BIT_HEADER_WRITE(hi2c->Devaddress));  // REWRITE:
      }
      else if (hi2c->EventCount == 1U)
      {
        /* Send header of slave address */
        // hi2c->Instance->DR = I2C_10BIT_HEADER_READ(hi2c->Devaddress);
        SetRegister(&(hi2c->Instance->DR), I2C_10BIT_HEADER_READ(hi2c->Devaddress));  // REWRITE:
      }
      else
      {
        /* Do nothing */
...
static void I2C_Master_ADD10(I2C_HandleTypeDef *hi2c)
{
  ...
  SetRegister(&(hi2c->Instance->DR), I2C_10BIT_ADDRESS(hi2c->Devaddress));  // REWRITE:
...
static void I2C_ITError(I2C_HandleTypeDef *hi2c)
{
  ...
  if (((CurrentMode == HAL_I2C_MODE_MASTER) || (CurrentMode == HAL_I2C_MODE_MEM)) && (CurrentState == HAL_I2C_STATE_BUSY_RX))
  {
    ...
    SetRegister(&(hi2c->Instance->CR1), GetRegister(&(hi2c->Instance->CR1)) & ~I2C_CR1_POS);  // REWRITE:
  }
...
static HAL_StatusTypeDef I2C_MasterRequestWrite(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint32_t Timeout, uint32_t Tickstart)
{
  ...
  else
  {
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_10BIT_HEADER_WRITE(DevAddress));  // REWRITE:
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_10BIT_ADDRESS(DevAddress));  // REWRITE:
  }
...
static HAL_StatusTypeDef I2C_MasterRequestRead(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint32_t Timeout, uint32_t Tickstart)
{
  ...
  if (hi2c->Init.AddressingMode == I2C_ADDRESSINGMODE_7BIT)
  {
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_7BIT_ADD_READ(DevAddress));  // REWRITE:
  }
  else
  {
    /* Send header of slave address */
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_10BIT_HEADER_READ(DevAddress));  // REWRITE:
  }
...
static HAL_StatusTypeDef I2C_RequestMemoryWrite(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint32_t Timeout, uint32_t Tickstart)
{
  ...
  if (MemAddSize == I2C_MEMADD_SIZE_8BIT)
  {
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_MEM_ADD_LSB(MemAddress));  // REWRITE:
  }
  /* If Memory address size is 16Bit */
  else
  {
    ...
    SetRegister(&(hi2c->Instance->DR), I2C_MEM_ADD_MSB(MemAddress));  // REWRITE:
...
static void I2C_DMAError(DMA_HandleTypeDef *hdma)
{
  ...
  if (HAL_DMA_GetError(hdma) != HAL_DMA_ERROR_FE)
  {
    ...
    SetRegister(&(hi2c->Instance->CR1), GetRegister(&(hi2c->Instance->CR1)) & ~I2C_CR1_ACK);  // REWRITE:
